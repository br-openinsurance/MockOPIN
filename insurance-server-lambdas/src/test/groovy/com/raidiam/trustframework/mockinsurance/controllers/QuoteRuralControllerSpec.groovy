...
@MicronautTest
class QuoteRuralControllerSpec  extends Specification {
    ...
    @MockBean(QuoteRuralLeadService)
    QuoteRuralLeadService quoteRuralLeadService() {
        Spy(QuoteRuralLeadService)
    }
    ...
    def "We can create a quote rural lead" () {
        given:
        def consentId = TestEntityDataFactory.aConsentId()
        def quote = TestEntityDataFactory.aQuoteRuralLead(consentId)
        quoteRuralLeadService.createQuote(_ as QuoteRuralLeadEntity) >> quote
        idempotencyRepository.findByIdempotencyId( _ as String) >> Optional.empty()

        def req = TestRequestDataFactory.createQuoteRuralLeadRequest()

        String json = mapper.writeValueAsString(req)
        def event = AwsProxyHelper.buildBasicEvent('/open-insurance/quote-rural/v1/lead/request', HttpMethod.POST)
                .withBody(json)
                .withHeaders(Map.of("x-idempotency-key", UUID.randomUUID().toString(), "x-fapi-interaction-id", UUID.randomUUID().toString()))
        AuthHelper.authorize(scopes: "quote-rural-lead", event)
    ...
    def "we can revoke a quote rural lead" () {
        given:
        def consentId = TestEntityDataFactory.aConsentId()
        def quote = TestEntityDataFactory.aQuoteRuralLead(consentId)
        quoteRuralLeadService.patchQuote(_ as RevokePatchPayload, _ as String, _ as String) >> quote

        def req = TestRequestDataFactory.revokeQuotePatchRequest(quote.getQuoteId())
        String json = mapper.writeValueAsString(req)

        def event = AwsProxyHelper.buildBasicEvent('/open-insurance/quote-rural/v1/lead/request/' + consentId, HttpMethod.PATCH)
                .withBody(json)
    ...
    def "We can create a quote rural lead v2" () {
        given:
        def consentId = TestEntityDataFactory.aConsentId()
        def quote = TestEntityDataFactory.aQuoteRuralLead(consentId)
        quoteRuralLeadService.createQuote(_ as QuoteRuralLeadEntity) >> quote
        ...
        String json = mapper.writeValueAsString(req)
        def event = AwsProxyHelper.buildBasicEvent('/open-insurance/quote-rural/v2/lead/request', HttpMethod.POST)
                .withBody(json)
                .withHeaders(Map.of("x-idempotency-key", UUID.randomUUID().toString(), "x-fapi-interaction-id", UUID.randomUUID().toString()))
        AuthHelper.authorize(scopes: "quote-rural-lead", event)
    ...
    def "we can revoke a quote rural lead v2" () {
        given:
        def consentId = TestEntityDataFactory.aConsentId()
        def quote = TestEntityDataFactory.aQuoteRuralLead(consentId)
        quoteRuralLeadService.patchQuote(_ as RevokePatchPayload, _ as String, _ as String) >> quote
        ...
        def event = AwsProxyHelper.buildBasicEvent('/open-insurance/quote-rural/v2/lead/request/' + consentId, HttpMethod.PATCH)
                .withBody(json)
                .withHeaders(Map.of("x-fapi-interaction-id", UUID.randomUUID().toString()))
        AuthHelper.authorize(scopes: "quote-rural-lead", event)
