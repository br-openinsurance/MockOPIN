...
class ConsentControllerSpec extends Specification {
    ...
    def "We can create a consent V3" () {
        given:
        def entity = TestEntityDataFactory.aConsent(UUID.randomUUID(), "random_client_id")
        consentService.createConsentV3(_ as CreateConsentV3, _ as String) >> entity
        idempotencyRepository.findByIdempotencyId( _ as String) >> Optional.empty()
        ...
        String json = mapper.writeValueAsString(req)
        def event = AwsProxyHelper.buildBasicEvent('/open-insurance/consents/v3/consents', HttpMethod.POST)
                .withBody(json)
                .withHeaders(Map.of("x-idempotency-key", UUID.randomUUID().toString()))
        AuthHelper.authorize(scopes: "consents", event)
    ...
    def "We can't create a consent V3 without an idempotency key" () {
        given:
        def entity = TestEntityDataFactory.aConsent(UUID.randomUUID(), "random_client_id")
        consentService.createConsentV3(_ as CreateConsentV3, _ as String) >> entity
    ...
    def "We can't create a consent V3 with an invalid idempotency key" () {
        given:
        def entity = TestEntityDataFactory.aConsent(UUID.randomUUID(), "random_client_id")
        consentService.createConsentV3(_ as CreateConsentV3, _ as String) >> entity
    ...
    def "We can't create a consent V3 with invalid idempotency payload" () {
        given:
        def entity = TestEntityDataFactory.aConsent(UUID.randomUUID(), "random_client_id")
        consentService.createConsentV3(_ as CreateConsentV3, _ as String) >> entity
    ...
    def "We can update a consent V3" () {
        ...
        def entity = TestEntityDataFactory.aConsent(accountHolderEntity.accountHolderId, "random_client_id")
        entity.accountHolder = accountHolderEntity
        consentService.updateConsentV3(_ as String, _ as UpdateConsent) >> entity.toResponseV3()
        def req = new UpdateConsent()

        String json = mapper.writeValueAsString(req)
        def event = AwsProxyHelper.buildBasicEvent('/open-insurance/consents/v3/consents/' + entity.getConsentId(), HttpMethod.PUT).withBody(json)
        AuthHelper.authorize(scopes: "op:consent", event)
    ...
    def "We can fetch a consent V3" () {
        given:
        def entity = TestEntityDataFactory.aConsent(UUID.randomUUID(), "random_client_id")
        consentService.getConsentV3(_ as String, _ as String) >> entity.toResponseV3()

        def event = AwsProxyHelper.buildBasicEvent('/open-insurance/consents/v3/consents/' + entity.getConsentId(), HttpMethod.GET)
        AuthHelper.authorize(scopes: "consents", event)
        ...
        response.statusCode == HttpStatus.OK.code
        response.body != null
        ResponseConsentV3 resp = mapper.readValue(response.body, ResponseConsentV3)
        resp.getData().getStatus() == EnumConsentStatus.AWAITING_AUTHORISATION
    ...
    def "We can fetch a full consent V3" () {
        given:
        def entity = TestEntityDataFactory.aConsent(UUID.randomUUID(), "random_client_id")
        consentService.getFullConsentV3(_ as String) >> entity.toResponseV3()
    ...
    def "We can delete a consent" () {
        ...
        def event = AwsProxyHelper.buildBasicEvent('/open-insurance/consents/v3/consents/random_consent_id', HttpMethod.DELETE)
        AuthHelper.authorize(scopes: "consents", event)
    ...
    def "A non-successful V3 response does not create an idempotency record" () {
        given:
        def idempotencyKey = UUID.randomUUID().toString()
        consentService.createConsentV3(_ as CreateConsentV3, _ as String) >> {
            throw new HttpStatusException(HttpStatus.UNPROCESSABLE_ENTITY, "UNPROCESSABLE_ENTITY: UNPROCESSABLE_ENTITY")
        }
        idempotencyRepository.findByIdempotencyId( _ as String) >> Optional.empty()
        def req = TestRequestDataFactory.createConsentV3Request(
