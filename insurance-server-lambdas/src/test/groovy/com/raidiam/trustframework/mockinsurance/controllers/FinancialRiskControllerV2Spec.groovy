...
@MicronautTest
class FinancialRiskControllerV2Spec extends Specification {
    @Inject
    ...
    def "We can fetch policies" () {
        ...
        financialRiskService.getPoliciesV2(_ as Pageable, _ as String) >> resp

        def event = AwsProxyHelper.buildBasicEvent('/open-insurance/insurance-financial-risk/v2/insurance-financial-risk', HttpMethod.GET)
    ...
    def "We can fetch a policy info" () {
        given:
        def resp = new ResponseInsuranceFinancialRiskPolicyInfoV2().data(
                new InsuranceFinancialRiskPolicyInfoDataV2()
        )
        InsuranceLambdaUtils.decorateResponseSimpleLinkMeta(resp::setLinks, resp::setMeta, "https://example.com")
        financialRiskService.getPolicyInfoV2(_ as UUID, _ as String) >> resp

        def event = AwsProxyHelper.buildBasicEvent('/open-insurance/insurance-financial-risk/v2/insurance-financial-risk/'+UUID.randomUUID().toString()+'/policy-info', HttpMethod.GET)
                .withHeaders(Map.of( "x-fapi-interaction-id", UUID.randomUUID().toString()))
        AuthHelper.authorizeAuthorizationCodeGrant(scopes: "insurance-financial-risk consent:urn:raidiaminsurance:bf43d0e5-7bc2-4a5b-b6da-19d43fabd991", event)
    ...
    def "We can fetch a policy's claims" () {
        given:
        def resp = new ResponseInsuranceFinancialRiskClaimsV2().data(List.of())
        InsuranceLambdaUtils.decorateResponseSimpleLinkMeta(resp::setLinks, resp::setMeta, "https://example.com")
        financialRiskService.getPolicyClaimsV2(_ as UUID, _ as String, _ as Pageable) >> resp

        def event = AwsProxyHelper.buildBasicEvent('/open-insurance/insurance-financial-risk/v2/insurance-financial-risk/'+UUID.randomUUID().toString()+'/claim', HttpMethod.GET)
                .withHeaders(Map.of( "x-fapi-interaction-id", UUID.randomUUID().toString()))
        AuthHelper.authorizeAuthorizationCodeGrant(scopes: "insurance-financial-risk consent:urn:raidiaminsurance:bf43d0e5-7bc2-4a5b-b6da-19d43fabd991", event)
    ...
    def "We can fetch a policy's premium" () {
        given:
        def resp = new ResponseInsuranceFinancialRiskPremiumV2().data(
                new InsuranceFinancialRiskPremiumV2()
        )
        InsuranceLambdaUtils.decorateResponseSimpleLinkMeta(resp::setLinks, resp::setMeta, "https://example.com")
        financialRiskService.getPolicyPremiumV2(_ as UUID, _ as String) >> resp

        def event = AwsProxyHelper.buildBasicEvent('/open-insurance/insurance-financial-risk/v2/insurance-financial-risk/'+UUID.randomUUID().toString()+'/premium', HttpMethod.GET)
                .withHeaders(Map.of( "x-fapi-interaction-id", UUID.randomUUID().toString()))
        AuthHelper.authorizeAuthorizationCodeGrant(scopes: "insurance-financial-risk consent:urn:raidiaminsurance:bf43d0e5-7bc2-4a5b-b6da-19d43fabd991", event)
