...
class FinancialRiskServiceSpecV2 extends CleanupFinancialRiskSpecification {
    ...
    def "we can get policies" () {
        when:
        def response = financialRiskService.getPoliciesV2(Pageable.from(0, 1), testConsent.getConsentId().toString())
    ...
    def "we can get a policy info" () {
        when:
        def response = financialRiskService.getPolicyInfoV2(testFinancialRiskPolicy.getFinancialRiskPolicyId(), testConsent.getConsentId().toString())
    ...
    def "we can get a policy's claims" () {
        when:
        def response = financialRiskService.getPolicyClaimsV2(testFinancialRiskPolicy.getFinancialRiskPolicyId(), testConsent.getConsentId().toString(), Pageable.from(0, 1))
    ...
    def "we can get a policy's premium" () {
        when:
        def response = financialRiskService.getPolicyPremiumV2(testFinancialRiskPolicy.getFinancialRiskPolicyId(), testConsent.getConsentId().toString())
