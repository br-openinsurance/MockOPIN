...
public class PatrimonialController extends BaseInsuranceController {
    ...
    @Get("/v{version}/insurance-patrimonial/{policyId}/premium")
    @ResponseErrorWithRequestDateTime
    @XFapiInteractionIdRequired
    @RequiredAuthenticationGrant(AuthenticationGrant.AUTHORISATION_CODE)
    public ResponseInsurancePatrimonialPremium getPremiumV1(@PathVariable("version") @Min(1) @Max(2) int version, @PathVariable("policyId") UUID policyId, HttpRequest<?> request) {
        var callerInfo = InsuranceLambdaUtils.getRequestMeta(request);
