...
@Entity
@Audited
@Table(name = "patrimonial_condominium_quotes")
public class QuotePatrimonialCondominiumEntity extends QuoteEntity {

    @Column(name = "data")
    @Type(JsonType.class)
    private QuoteData data;

    public static QuotePatrimonialCondominiumEntity fromRequest(QuoteRequestPatrimonialCondominium req, String clientId) {
        var entity = new QuotePatrimonialCondominiumEntity();
        ...
        var data = new QuotePatrimonialCondominiumEntity.QuoteData();
        data.setV1(req.getData());
        entity.setData(data);
        return entity;
    }

    public static QuotePatrimonialCondominiumEntity fromRequestV2(QuoteRequestPatrimonialCondominiumV2 req, String clientId) {
        var entity = new QuotePatrimonialCondominiumEntity();
    ...
    public ResponseQuotePatrimonialCondominium toResponse() {
        var quoteData = new ResponseQuotePatrimonialCondominiumData();
        ...
        if (QuoteStatusEnum.ACPT.toString().equals(this.getStatus())) {
            ...
            var quote = new QuotePatrimonialCondominiumQuotes();
            ...
            var quoteInfo = new QuotePatrimonialCondominium();
        ...
        var resp = new ResponseQuotePatrimonialCondominium();
        resp.setData(quoteData);
        return resp;
    }

    public ResponseQuotePatrimonialCondominiumV2 toResponseV2() {
        var quoteData = new ResponseQuotePatrimonialCondominiumV2Data();
        quoteData.setStatus(QuoteStatusEnum.fromValue(this.getStatus()));
        quoteData.setStatusUpdateDateTime(InsuranceLambdaUtils.dateToOffsetDate(this.getUpdatedAt()));
        ...
        if (QuoteStatusEnum.ACPT.toString().equals(this.getStatus())) {
            ...
            var quoteInfo = new QuotePatrimonialCondominiumV2();
            quoteInfo.setQuoteCustomer(customer);
        ...
        var resp = new ResponseQuotePatrimonialCondominiumV2();
        resp.setData(quoteData);
        return resp;
    }
    ...
    public boolean shouldReject() {
        if (this.getData().getV1() != null) {
            return Optional.ofNullable(this.getData().getV1())
                    .map(QuotePatrimonialCondominiumData::getQuoteData)
                    .map(QuoteDataPatrimonialCondominium::getPolicyId).isEmpty();
        }
        return Optional.ofNullable(this.getData().getV2())
                .map(QuotePatrimonialCondominiumDataV2::getQuoteData)
                .map(QuoteDataPatrimonialCondominium::getPolicyId).isEmpty();
    }
    ...
    public static class QuoteData implements Serializable {
        @JsonProperty("v1")
        private QuotePatrimonialCondominiumData v1;
        @JsonProperty("v2")
        private QuotePatrimonialCondominiumDataV2 v2;
